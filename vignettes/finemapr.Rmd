...
```{r run_finemap, eval=FALSE}
options(finemapr_finemap = "~/apps/finemap/finemap")

out_finemap <- run_finemap(z1, ld1, n1, args = "--n-causal-max 3")
```

```{r print_finemap, eval=FALSE}
print(out_finemap)
```

```{r plot_finemap, fig.width = 6, fig.height = 9, eval=FALSE}
plot(out_finemap, label_size = 3, grid_ncol = 1)
```

## Run CAVIAR

```{r run_caviar, eval=FALSE}
options(finemapr_caviar = "~/apps/caviar/CAVIAR")

out_caviar <- run_caviar(z1, ld1, args = "-c 3")
```

```{r print_caviar, eval=FALSE}
print(out_caviar)
```

```{r plot_caviar, fig.width = 6, fig.height = 3, eval=FALSE}
plot(out_caviar, label_size = 3)
```

## Run PAINTOR

```{r run_paintor, eval=FALSE}
options(finemapr_paintor = "~/apps/paintor/PAINTOR")

out_paintor <- run_paintor(z1, ld1, n1, args = "-enumerate 3")
```

```{r print_paintor, eval=FALSE}
print(out_paintor)
```

```{r plot_paintor, fig.width = 6, fig.height = 3, eval=FALSE}
